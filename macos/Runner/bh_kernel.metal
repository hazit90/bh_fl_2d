...
// Texture-writing variant (BGRA8). Writes directly to a texture for zero-copy.
kernel void bh_kernel_tex(
  ...
  uint2 gid [[thread_position_in_grid]])
{
  ...
  float4 color = float4((float)outc.x / 255.0f, (float)outc.y / 255.0f, (float)outc.z / 255.0f, (float)outc.w / 255.0f);
